...
import cn.itcast.handheldclass.yzm.Code;
...
//        //将验证码用图片的形式显示出来
    ...
    public void change_sure(View view) {
        ...
        if(Login_name.equals("") || phone.equals("")|| new_pwd1.equals("")|| new_pwd2.equals("")||TextUtils.isEmpty(phoneCodes)){
            Toast.makeText(ForgetPwdActivity.this,"填写信息不能为空",Toast.LENGTH_SHORT).show();
        }else {
            ...
            if(!(phoneCodes.equals(realCode))){
                ...
                if(index == 0){
                    ...
                    if(ctrInfo(Login_name,phone)){
                        ...
                        Toast.makeText(ForgetPwdActivity.this,"用户名或手机号错误",Toast.LENGTH_SHORT).show();
                    }
                }else if(index == 1){
                    ...
                    sql = "select * from company where username = '"+Login_name+"' and number = '"+phone+"'";
                    if(ctrInfo(Login_name,phone)){
                        String sq2 = "update company set pwd = '"+new_pwd1+ "' where username = '" + Login_name + "'";
                        db.execSQL(sq2);
                        Intent intent = new Intent(this,LoginActivity.class);
                        startActivity(intent);
                        Toast.makeText(ForgetPwdActivity.this,"密码修改成功",Toast.LENGTH_SHORT).show();
